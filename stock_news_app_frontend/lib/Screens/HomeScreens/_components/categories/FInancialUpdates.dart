...
class Financialupdates extends StatefulWidget {
  const Financialupdates({super.key});

  @override
  State<Financialupdates> createState() => _FinancialupdatesState();
}

class _FinancialupdatesState extends State<Financialupdates> {
...
bool end = false;
  void fetchPosts() async {
    ...
    final req = jsonEncode(
        {"userEmail": FirebaseAuth.instance.currentUser!.email as String, "category": "Financial Update"});
    ...
    final duration = endTime.difference(start).inMilliseconds;    await analytics.logEvent(name: "home_load_time_finance", parameters: {
      "time": duration
    });
    ...
    await analytics.logEvent(name: "Category", parameters: {
      "category_name": "Financial Update",
